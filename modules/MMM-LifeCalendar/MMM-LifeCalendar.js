Module.register("MMM-LifeCalendar", {
  defaults: {
    ...
    colorScheme: "green", // Color Schemes: {default, gray, green, neutral}
    icons_display: false,
    duration_events: [
  ...
  getDom: function () {
    ...
    const weeksLived = this.getAge(); // Retain this line to keep the functionality
    ...
    for (let i = 0; i < this.config.totalYears; i++) {
      ...
      for (let j = 0; j < this.config.weeksPerYear; j++) {
        ...
        let colorSchemeSuffix = "";
        if (this.config.colorScheme === "gray") {
          colorSchemeSuffix = "_gray";
        } else if (this.config.colorScheme === "green") {
          colorSchemeSuffix = "_green";
        } else if (this.config.colorScheme === "beige") {
          colorSchemeSuffix = "_beige";
        } else if (this.config.colorScheme === "neutral") {
          colorSchemeSuffix = "_neutral";
        }
        ...
        this.config.date_events.forEach(event => {
          if (i * this.config.weeksPerYear + j === event.duration_weeks) {
            ...
            //Adding the icon
            if (this.config.icons_display){
              const icon = document.createElement("img");        
              icon.src = "modules/MMM-LifeCalendar/Icons/" + event.icon_name;
              icon.className="icon";
